...
namespace uSync.Core.Serialization.Serializers
{
        ...
        where TObject : IContentTypeComposition
    {
        ...
        protected virtual XElement SerializeProperties(TObject item)
        {
            ...
            foreach (var property in item.PropertyTypes.Cast<PropertyType>().OrderBy(x => x.Alias))
            {
            ...
            out bool IsNew)
        {
            ...
            if (key != Guid.Empty)
            {
                ...
                property = (PropertyType)item.PropertyTypes.SingleOrDefault(x => x.Key == key);
            }
            ...
            if (property == null)
            {
                ...
                property = (PropertyType)item.PropertyTypes.SingleOrDefault(x => x.Alias == alias);
            }
