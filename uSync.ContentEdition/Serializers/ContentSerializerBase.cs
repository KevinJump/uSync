        ...
        where TObject : IContentBase
    {
        ...
        protected virtual XElement SerializeProperties(TObject item, SyncSerializerOptions options)
        {
                ...
                .OrderBy(x => x.Alias))
            {
                ...
                foreach (var value in property.Values.Cast<PropertyValue>())
                {
        ...
        public virtual string GetItemPath(TObject item)
        {
            ...
            if (item.Trashed)
            {
                ...
                if (parent != null)
                {
                    return GetItemPath(parent) + "/" + item.Name.ToSafeAlias(shortStringHelper);
                }
            }
        ...
        protected virtual string GetItemPath(IEntitySlim item)
        {
            ...
            pathCache[item.Path] = path + "/" + item.Name.ToSafeAlias(shortStringHelper);
